...
"""Shared 6-camera input handling for the MindDrive ROS node.

MindDrive is a surround-view model (the same rig as MindDrive): ``MinddriveAgent.sensors()`` spawns six cameras and
``MinddriveAgent.tick()`` hands the pipeline six *different* images, one per mount.
...
This module owns the multi-camera plumbing (the same implementation as
orion_ros.camera_input, which it was copied from):
...
DEFAULT_CAMERA_TOPICS: List[str] = [
    f"/carla/hero/{cam}/image" for cam in CAMERA_ORDER
]
...
def decode_image(record: tuple, jpeg_quality: int = 20) -> np.ndarray:
    ...
    BGR (not RGB) is deliberate: the MindDrive agent feeds BGR and lets the pipeline's
...
class MultiCameraBuffer:
    """Latest-frame buffers for the six MindDrive cameras.
    ...
    def __init__(
        ...
        if len(topics) == 1:
            ...
            self._logger.warn(
                "SINGLE-CAMERA MODE: '%s' will be replicated into all 6 MindDrive "
                ...
                "rear and side views. This is NOT faithful to MinddriveAgent and any "
                "Bench2Drive score from it is not comparable — pass 6 topics via "
                "the 'camera_topics' parameter for the real setup." % topics[0]
            )
        elif len(topics) == len(CAMERA_ORDER):
            self.replicate = False
            self._topics = dict(zip(CAMERA_ORDER, topics))
        else:
            raise ValueError(
                f"camera_topics must have exactly 1 (replicate) or "
                f"{len(CAMERA_ORDER)} entries in MindDrive order "
                f"{CAMERA_ORDER}; got {len(topics)}"
            )
    ...
    def _make_callback(self, cam: str):
        def _cb(msg: RosImage) -> None:
            ...
            self.received_at[cam] = time.monotonic()
        return _cb
    ...
    def oldest_receive_time(self) -> Optional[float]:
        """time.monotonic() of the OLDEST of the latest records, or None while
        any camera is still missing."""
        if self.missing():
            return None
        return min(self.received_at[c] for c in self._topics)
    ...
    def snapshot(self) -> Optional[Dict[str, tuple]]:
        """Latest record for every MindDrive camera, or None if any is still missing.
        ...
        if self.replicate:
            ...
            return {cam: rec for cam in CAMERA_ORDER}
